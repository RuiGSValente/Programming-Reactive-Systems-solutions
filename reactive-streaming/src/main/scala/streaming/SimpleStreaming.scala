...
object SimpleStreaming extends ExtraStreamOps {
  ...
  def mapToStrings(ints: Source[Int, NotUsed]): Source[String, NotUsed] = ints.via(Flow[Int].map(_.toString))
  ...
  //Source[Int].runWith(Sink.fold(0)(_ + _))
